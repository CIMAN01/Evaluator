...
public class StringSplitter {
    ...
    public StringSplitter(String line) {
        ...
        // private helper method for finding the next token
        findNextToken();
    }
    ...
    private void findNextToken() {
        // While queue is not empty and there is whitespace, remove it
        while (!characters.isEmpty() &&
                Character.isWhitespace(characters.peek())) {
            characters.remove();
        }
        // if there are no more characters left we set token to null.
        if (characters.isEmpty()) {
            token = null;
        } else {
            // add character to token
            token = "" + characters.remove();
            // if token does not contain any special characters
            if (!SPECIAL_CHARACTERS.contains(token)) {
                boolean done = false;
                // while there are still characters left and done != true
                while (!characters.isEmpty() && !done) {
                    // peek at the next character
                    char ch = characters.peek();
                    // if we find empty space or special characters we exit the loop
                    if (Character.isWhitespace(ch) ||
                            SPECIAL_CHARACTERS.indexOf(ch) >= 0) {
                        done = true;
                    // else we add the next token
                    } else {
